...
class APP_SY_Frame(ctk.CTkFrame):
    def __init__(self , master):
        ...
        self.CM2_M1_M2_Path = []
        self.period = None
        self.Line1 =  np.array([[477, 1070], [1268, 230], [1543, 352], [996, 1079]])
        self.Line2 = np.array([[1754, 802], [1614, 1030], [583, 425], [791, 279]])
        self.track_color = {}
    ...
    def Main_Save_path(self, frame, ID_camala, timestamp ):
        ...
        if self.CM1_Model1_list and self.CM2_Model1_list:
            # ดึงข้อมูล timestamp ล่าสุดจากแต่ละ list
            time_str1 = self.CM1_Model1_list[-1]['timestamp']
            time_str2 = self.CM2_Model1_list[-1]['timestamp']
            ...
            if time_diff <= 3:
                ...
                timeSQL = time1.strftime('%H:%M:%S')
                timePATH = time1.strftime('%H-%M-%S')
                full_save_dir = f"{save_dir}/{timePATH}"  # ✅ เพิ่มตรงนี้
                os.makedirs(full_save_dir, exist_ok=True)  # ✅ เพิ่มตรงนี้
                
                for i, item in enumerate(self.CM1_Model1_list):
                    C1_M1_Path_SQL = f"{full_save_dir}/_C1_Model1_{i}_{unique_id}.jpg"
                ...
                for i, item in enumerate(self.CM2_Model1_list):
                    C2_M1_Path_SQL = f"{full_save_dir}/_C2_Model1_{i}_{unique_id}.jpg"
                ...
                for i, item in enumerate(self.CM1_Model2_list):
                    C1_M2_Path_SQL = f"{full_save_dir}/_C_one_Model2_{i}_{unique_id}.jpg"
                ...
                for i, item in enumerate(self.CM2_Model2_list):
                    C2_M2_Path_SQL = f"{full_save_dir}/_C_two_Model2_{i}_{unique_id}.jpg"
                ...
                self.My_show_img1_cap2.configure(image="")
                ...
                self.My_show_img2_cap2.configure(image="")
                self.My_show_img2_cap2.image = None
                
                self.My_show_img2.configure(image = "")
