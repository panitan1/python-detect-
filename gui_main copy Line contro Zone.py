...
class APP_SY_Frame(ctk.CTkFrame):
    def __init__(self , master):
        ...
        self.master = master
        self.cam01 = cv2.VideoCapture("vidio\\timeNO.mp4")
        self.cam02 = cv2.VideoCapture("vidio\\timeNO2.mp4")
        ...
        self.CM2_M1_M2_Path = []
        self.period = None
        self.Line1 =  np.array([[90, 533], [1289, 120], [1430, 249], [154, 676]])
        self.Line2 = np.array([[876, 340], [852, 407], [279, 202], [346, 153]])
    ...
    def Process_Cam01(self):
        ...
        while self.running and self.save_lock:
            ...
            cv2.fillPoly(draw_frame, [self.Line1], color=(0, 255, 0))
            alpha = 0.3
            frame = cv2.addWeighted(draw_frame, alpha, frame, 1 - alpha, 0)
            cv2.polylines(frame, [self.Line1], isClosed=True, color=(0, 123, 44), thickness=3)
            ...
            if annotated_frame1.boxes is not None:
                ...
                for box in annotated_frame1.boxes:
                    ...
                    if box.id is not None:
                        ...
                        if (self.im_SAVE_Id_model1_cm1 != self.im_save_CHECK_Id_model1_cm1 and cv2.pointPolygonTest(self.Line1, (float(cx), float(cy)), False) >= 0 )or self.Have_cam2_no_cam1 == True:
    ...
    def Process_Cam02(self):
        ...
        while self.running and self.save_lock2:
            ...
            draw_frame = frame2.copy()
            cv2.fillPoly(draw_frame, [self.Line2], color=(0, 255, 0)) 
            alpha = 0.3
            frame2 = cv2.addWeighted(draw_frame, alpha, frame2, 1 - alpha, 0)
            cv2.polylines(frame2, [self.Line2], isClosed=True, color=(0, 123, 44), thickness=3)
            ...
            if annotated_frame1.boxes is not None:
                ...
                for box in annotated_frame1.boxes:
                    ...
                    if box.id is not None:
                        ...
                        if (self.im_SAVE_Id_model1_cm2 != self.im_save_CHECK_Id_model1_cm2 and cv2.pointPolygonTest(self.Line2, (float(cx), float(cy)), False) >= 0 )or self.Have_cam1_no_cam2 == True:
