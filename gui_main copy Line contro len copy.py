...
class APP_SY_Frame(ctk.CTkFrame):
            ...
            if annotated_frame1.boxes is not None:
                ...
                for box in annotated_frame1.boxes:
                    ...
                    if box.id is not None:
                        ...
                        if ( track_id not in  self.im_SAVE_Id_model1_cm1 != self.im_save_CHECK_Id_model1_cm1  and self.is_Check_LINE1(cx, cy, self.Line1[0], self.Line1[1])) or self.Have_cam2_no_cam1 == True:
                            if track_id in self.counter1:
                                ...
                                if cx > prev_cx:
                                    ...
                                    print(f"[LOG] ID {track_id}✅✅ crossed the line from {direction}")
                                elif cx < prev_cx:
    ...
    def Main_Save_path(self, frame, ID_camala, timestamp ,path):
        ...
        if self.Have_cam1_no_cam2 == True and  self.Have_cam2_no_cam1 == True  :
            ...
            if self.direction_against001 == True or self.direction_against002 == True:
    ...
    def start_cam(self):
        ...
        multiprocessing.Process(target= self.Process_Cam01).start()
        multiprocessing.Process(target= self.Process_Cam02).start()
